...
class IP_MAMDataset(BaseImageDataset):
    def __init__(self,
            zarr_path, 
            horizon=1,
            pad_before=0,
        ...
        self.replay_buffer = ReplayBuffer.copy_from_path(
            zarr_path, keys=['img1', 'img2', 'state', 'action', 'MAS'])
    ...
    def get_normalizer(self, mode='limits', **kwargs): #condition已经归一化过了
        ...
        # 处理MAS的归一化
        normalizer['MAS'] = SingleFieldLinearNormalizer.create_identity() # 100*8
    ...
    def _sample_to_data(self, sample):
        ...
        # 处理MAS的维度
        T = len(agent_pos)
        MAS = sample['MAS'].astype(np.float32)   #（timestep, 8）
        #MAS = condition.reshape(T, -1)  # (T, 800)

        #加padding MAS根据每个dataset的最长timestep（l），加上padding （-2）， 形状为（l，8）
        #action 
