...
class MaskSFT:
    def __init__(self, input_path, output_path, mask_type, retain_ratio=None, size=1, normalize=False):
    ...
    def mask_action(self, action, retain_ratio=None):
        ...
        if self.mask_type == '2D_video_trajectory':
            ...
            # 保留所有time（第7列）数据，随机保留retain_ratio比例的x,y,z,dx,dy,dz,gripper（0-6列）
            if retain_ratio is None:
                raise ValueError('local_planner类型需要retain_ratio参数')
            n = action.shape[0]
            action[:, :] = -1
            action[:, 7] = self._original_action[:, 7]
            retain_num = int(n * retain_ratio)
            idx = np.arange(n)
            np.random.shuffle(idx)
            retain_idx = idx[:retain_num]
...
def normalize_actions_to_new_h5(input_path, output_path, json_path):
    """
    归一化所有trajectory下group的action的0-6列，区间(-1,1]，写入新h5，max/min写入json。
    """
    mins = np.full(7, np.inf)
    maxs = np.full(7, -np.inf)
    with h5py.File(input_path, 'r') as f:
...
def main():
    ...
    masker = MaskSFT(args.input, args.output, args.mask_type, args.retain_ratio, args.size, args.normalize)
    masker.run()
