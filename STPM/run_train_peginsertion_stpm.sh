...
REPLAY_DATASET_PATH="${REPLAY_DATASET_PATH:-demos/PegInsertionSide-v1/PegInsertionSide-v1.rgb.pd_ee_pose.physx_cpu.h5}"
AUTO_REPLAY="${AUTO_REPLAY:-false}"

if [[ ! -f "${REPLAY_DATASET_PATH}" ]]; then
  if [[ "${AUTO_REPLAY}" == "true" ]]; then
    echo "[peginsertion] replay dataset not found, start replay: ${REPLAY_DATASET_PATH}"
    ...
    echo "  NUM_ENVS=10 bash examples/baselines/diffusion_policy/tmp/run_peginsertion_replay_full.sh" >&2
    exit 1
  fi
fi

DATASET_PATH="${DATASET_PATH:-${REPLAY_DATASET_PATH}}" \
TASK_NAME="${TASK_NAME:-peginsertion}" \
TASK_DESCRIPTION="${TASK_DESCRIPTION:-insert the peg into the horizontal box hole}" \
OUTPUT_DIR="${OUTPUT_DIR:-STPM_peginsertion}" \
