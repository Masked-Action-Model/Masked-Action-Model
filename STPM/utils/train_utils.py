...
try:
    ...
    from STPM.utils.normalizer import SingleFieldLinearNormalizer
...
@torch.no_grad()
def get_normalizer_from_calculated(path, device, state_dim=None) -> "SingleFieldLinearNormalizer":
    """
    ...
    def to_tensor_slice(data):
        if state_dim is None:
            ...
            if len(data) < state_dim:
                raise ValueError(
                    f"Normalizer stats only contain {len(data)} dims, but state_dim={state_dim} was requested."
                )
            slice_data = data[:state_dim]
        return torch.tensor(slice_data, dtype=torch.float32, device=device)
