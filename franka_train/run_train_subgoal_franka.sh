...
# 2. Raw paths and train-time mask preprocess params
RAW_DEMO_H5="${RAW_DEMO_H5:-franka_train/data/franka_real.h5}"
PREPROCESSED_DATA_DIR="${PREPROCESSED_DATA_DIR:-franka_train/data}"
PREPROCESSED_DATA_PREFIX="${PREPROCESSED_DATA_PREFIX:-franka_real_subgoal}"
...
if [[ "$need_preprocess" == "true" ]]; then
  ...
  PREPROCESS_ARGS=(
    ...
    --output-h5 "$DEMO_PATH"
    --env-id "$ENV_ID"
    --control-mode "${CONTROL_MODE:-pd_ee_pose}"
  ...
  if [[ -n "$MASK_TYPE_LIST" ]]; then
    PREPROCESS_ARGS+=(--mask-type-list "$MASK_TYPE_LIST")
  fi
  if [[ -n "$MASK_RATIO_LIST" ]]; then
    PREPROCESS_ARGS+=(--mask-ratio-list "$MASK_RATIO_LIST")
  fi
  if [[ -n "$MASK_COMPOSITION_LIST" ]]; then
    PREPROCESS_ARGS+=(--mask-composition-list "$MASK_COMPOSITION_LIST")
  fi
  if [[ -n "$TRAIN_NUM_MASK_TYPE" ]]; then
    PREPROCESS_ARGS+=(--train-num-mask-type "$TRAIN_NUM_MASK_TYPE")
  fi
  if [[ -n "$TRAIN_MASK_TYPE_LIST" ]]; then
    PREPROCESS_ARGS+=(--train-mask-type-list "$TRAIN_MASK_TYPE_LIST")
  fi
  if [[ -n "$TRAIN_MASK_RATIO_LIST" ]]; then
    PREPROCESS_ARGS+=(--train-mask-ratio-list "$TRAIN_MASK_RATIO_LIST")
  fi
  if [[ -n "$TRAIN_MASK_COMPOSITION_LIST" ]]; then
    PREPROCESS_ARGS+=(--train-mask-composition-list "$TRAIN_MASK_COMPOSITION_LIST")
  fi
  ...
  echo "[subgoal-preprocess] write ${DEMO_PATH}"
...
DEMO_TYPE="${DEMO_TYPE:-franka_real_subgoal}"

# 6. Logging/checkpointing. Eval args are accepted for parity; no online eval runs.
...
if [[ -n "$EVAL_DEMO_PATH" ]]; then
  ARGS+=(--eval-demo-path "$EVAL_DEMO_PATH")
fi
...
"$PYTHON_BIN" franka_train/train_subgoal_franka.py "${ARGS[@]}"
