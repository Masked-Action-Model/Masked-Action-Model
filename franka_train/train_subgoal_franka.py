...
class Args:
    ...
    demo_type: Optional[str] = "franka_real_subgoal"


def main() -> None:
    args = tyro.cli(Args)
    args.action_dim = infer_action_dim(args.demo_path, args.action_dim)
    mas_max_length = subgoal_mod.read_mas_max_length(args.demo_path)
    args.subgoal_dim = int(mas_max_length) * int(args.action_dim)
