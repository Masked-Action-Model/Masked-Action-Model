...
def main():
    ...
    with h5py.File(args.orig, "r") as f_orig, h5py.File(args.masked, "r") as f_masked, h5py.File(args.out, "w") as f_out:
        # meta
        if "meta" in f_masked:
        ...
        common_trajs = [t for t in masked_trajs if t in orig_trajs]
        if not common_trajs:
            raise RuntimeError("No common traj_* groups between orig and masked files.")
        ...
        for t in common_trajs:
            ...
            # mask is computed from masked mas, then trimmed to original action length (no padding in output mask)
            full_mask = (mas != padding_value).astype(np.float32)
            orig_actions = f_orig[t]["actions"][...]
            if orig_actions.shape[0] > full_mask.shape[0]:
                raise RuntimeError(
                    f"{t}: original actions length {orig_actions.shape[0]} > masked length {full_mask.shape[0]}"
                )
            mask = full_mask[: orig_actions.shape[0]]
            g_out.create_dataset(args.mask_name, data=mask, dtype=np.float32)

            # actions come directly from original file (no padding)
            g_out.create_dataset(args.actions_name, data=orig_actions, dtype=orig_actions.dtype)
