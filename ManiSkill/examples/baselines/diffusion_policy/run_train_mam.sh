...
# Train script for examples/baselines/diffusion_policy/train_mam.py
...
EVAL_DATA_PATH="${EVAL_DATA_PATH:-demos/data_1/data_1_concat_eval.h5}"
NUM_DEMOS="${NUM_DEMOS:-50}"
NUM_EVAL_DATA="${NUM_EVAL_DATA:-10}"
...
UNET_DIMS_1="${UNET_DIMS_1:-64}"
UNET_DIMS_2="${UNET_DIMS_2:-128}"
UNET_DIMS_3="${UNET_DIMS_3:-256}"
N_GROUPS="${N_GROUPS:-8}"
OBS_MAS_DIM="${OBS_MAS_DIM:-}"
...
ARGS=(
  ...
  --unet-dims "$UNET_DIMS_1" "$UNET_DIMS_2" "$UNET_DIMS_3"
  --n-groups "$N_GROUPS"
  --log-freq "$LOG_FREQ"
  --eval-freq "$EVAL_FREQ"
...
if [[ -n "$EVAL_DATA_PATH" ]]; then
  ARGS+=(--eval-data-path "$EVAL_DATA_PATH")
fi
if [[ -n "$NUM_DEMOS" ]]; then
  ARGS+=(--num-demos "$NUM_DEMOS")
fi
if [[ -n "$NUM_EVAL_DATA" ]]; then
  ARGS+=(--num-eval-data "$NUM_EVAL_DATA")
fi
if [[ -n "$OBS_MAS_DIM" ]]; then
  ARGS+=(--obs-mas-dim "$OBS_MAS_DIM")
fi
