...
from mani_skill.utils.wrappers.flatten import FlattenRGBDMasObservationWrapper
...
class Args:
    ...
    """path to evaluation h5 dataset (uses corresponding .json with same basename)"""
    num_demos: Optional[int] = None
    """number of trajectories to load from the demo dataset"""
    num_eval_data: Optional[int] = None
    """number of trajectories to load from the eval dataset"""
    ...
    obs_mas_dim: Optional[int] = None
    """dimension of flattened mas features used in obs conditioning (set from dataset if None)"""
...
class SmallDemoDataset_DiffusionPolicy(Dataset):  # Load everything into memory
    def __init__(self, data_path, obs_process_fn, obs_space, device, num_traj):
        ...
        # Action normalization bounds for pd_ee_pose:
    ...
    def __getitem__(self, index):
        ...
        act_seq = (act_seq - 0.5 * (self.action_high + self.action_low)) / (
...
class Agent(nn.Module):
    def __init__(self, env: VectorEnv, args: Args):
        ...
        assert args.obs_mas_dim is not None and args.obs_mas_dim > 0, "obs_mas_dim must be set for MAM conditioning"
        obs_mas_dim = args.obs_mas_dim
    ...
    def encode_obs(self, obs_seq, eval_mode):
        ...
        feature = torch.cat((visual_feature, obs_seq["state"], obs_seq["mas"]), dim=-1)
    ...
    def get_action(self, obs_seq, mask_seq):
        # init scheduler
        ...
        with torch.no_grad():
            ...
            a0_seq = obs_seq["mas"][:, : self.pred_horizon].to(
...
if __name__ == "__main__":
    ...
    envs = make_eval_envs(
        ...
        wrappers=[FlattenRGBDMasObservationWrapper],
    )
    ...
    if args.obs_mas_dim is None:
        args.obs_mas_dim = dataset.mas_flat_dim
    ...
    def evaluate_and_save_best(iteration):
        ...
        if iteration % args.eval_freq == 0:
            ...
            eval_metrics = evaluate(
                ...
                device,
                args.sim_backend,
                eval_h5_path=args.eval_data_path,
                env_id=args.env_id,
            ...
            for k in ["mse_unknown", "mse_all"]:
                if k in eval_metrics and (k not in best_eval_metrics or eval_metrics[k] < best_eval_metrics[k]):
                    best_eval_metrics[k] = eval_metrics[k]
                    save_ckpt(run_name, f"best_eval_{k}")
                    print(f"New best {k}: {eval_metrics[k]:.6f}. Saving checkpoint.")
