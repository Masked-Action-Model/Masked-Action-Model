...
# Train script for examples/baselines/diffusion_policy/train_rgbd.py
# Override any value by exporting the variable before running this script.

EXP_NAME="${EXP_NAME:-PickCube_rgbd}"
SEED="${SEED:-1}"
...
TOTAL_ITERS="${TOTAL_ITERS:-200000}"
...
if [[ -z "$ACTION_NORM_PATH" ]]; then
  ...
  echo "  bash examples/baselines/diffusion_policy/run_train_rgbd.sh" >&2
  exit 2
fi
...
python examples/baselines/diffusion_policy/train_rgbd.py "${ARGS[@]}"
