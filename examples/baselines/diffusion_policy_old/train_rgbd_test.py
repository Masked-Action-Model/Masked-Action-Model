...
from diffusion_policy.evaluate_test import evaluate
...
class Args:
    ...
    """number of demos selected from loaded demos for evaluation"""
    eval_demo_selection_seed: Optional[int] = None
    """seed used to select eval demos from loaded demos. defaults to args.seed"""
    ...
    eval_reset_seed: Optional[int] = None
    """If set, force evaluation env reset(seed=eval_reset_seed). If None, auto-infer from demo metadata when possible."""
...
def _candidate_metadata_paths(demo_path: str):
    ...
    for s in suffixes:
        if base.endswith(s):
            candidates.append(f"{base[:-len(s)]}.json")
...
def infer_eval_reset_seed_from_demo(demo_path: str):
    """Infer the reset seed for the first trajectory loaded from demo_path (traj with smallest id)."""
    ...
    first_episode_id = int(traj_keys[0].split("_")[-1])

    for meta_path in _candidate_metadata_paths(demo_path):
        ...
        try:
            ...
            if first_episode_id >= len(episodes):
                continue
            ep = episodes[first_episode_id]
            reset_seed = ep.get("reset_kwargs", {}).get("seed", None)
            if isinstance(reset_seed, list):
                reset_seed = reset_seed[0] if len(reset_seed) > 0 else None
            if reset_seed is None:
                reset_seed = ep.get("episode_seed", None)
            if reset_seed is not None:
                print(
                    f"[seed-infer] using seed={reset_seed} from {meta_path} for {traj_keys[0]}"
                )
...
def infer_eval_reset_kwargs_list_from_demo(
    demo_path: str, num_traj: Optional[int] = None
) -> List[dict]:
    traj_ids = infer_loaded_traj_ids_from_demo(demo_path, num_traj=num_traj)
    if len(traj_ids) == 0:
        return []
    ...
    for meta_path in _candidate_metadata_paths(demo_path):
        ...
        try:
            ...
            if len(reset_kwargs_list) == len(traj_ids):
                print(
                    f"[seed-infer] loaded {len(reset_kwargs_list)} eval reset kwargs from {meta_path}"
                )
                return reset_kwargs_list
        except Exception as e:
            print(f"[seed-infer] failed reading {meta_path}: {e}")
    print("[seed-infer] failed to infer evaluation reset kwargs list from demo metadata")
    return []


def select_eval_demo_indices(
    total_demos: int, num_eval_demos: int, selection_seed: int
) -> List[int]:
    if total_demos <= 0 or num_eval_demos <= 0:
        return []
    count = min(total_demos, num_eval_demos)
    rng = np.random.default_rng(selection_seed)
    selected = rng.choice(total_demos, size=count, replace=False)
...
class Agent(nn.Module):
    ...
    def get_action(self, obs_seq):
        ...
        # If demo actions were min-max normalized to [-1, 1], map model outputs
        # back to controller's physical action range for env.step().
...
if __name__ == "__main__":
    ...
    eval_selection_seed = (
        args.seed if args.eval_demo_selection_seed is None else args.eval_demo_selection_seed
    )
    loaded_traj_ids = infer_loaded_traj_ids_from_demo(args.demo_path, num_traj=args.num_demos)
    eval_demo_indices = select_eval_demo_indices(
        total_demos=len(loaded_traj_ids),
        num_eval_demos=args.num_eval_demos,
        selection_seed=eval_selection_seed,
    )
    print(
        f"[eval-demo] selected {len(eval_demo_indices)} / {len(loaded_traj_ids)} demos "
        f"for eval with selection_seed={eval_selection_seed}: {eval_demo_indices}"
    )

    eval_reset_kwargs_list = []
    inferred_reset_kwargs_list = infer_eval_reset_kwargs_list_from_demo(
        args.demo_path, num_traj=args.num_demos
    )
    if len(inferred_reset_kwargs_list) > 0 and len(eval_demo_indices) > 0:
        eval_reset_kwargs_list = [inferred_reset_kwargs_list[i] for i in eval_demo_indices]
        print(
            "[eval-demo] selected eval reset seeds: "
            f"{[kwargs.get('seed', None) for kwargs in eval_reset_kwargs_list]}"
        )

    eval_reset_seed = args.eval_reset_seed
    if len(eval_reset_kwargs_list) == 0:
        if eval_reset_seed is None:
            eval_reset_seed = infer_eval_reset_seed_from_demo(args.demo_path)
            if eval_reset_seed is None:
                print("[seed-infer] fallback: evaluation reset will not be seeded")
            else:
                print(f"[seed-infer] auto inferred eval_reset_seed={eval_reset_seed}")
        if eval_reset_seed is not None and args.num_eval_envs > 1:
            print(
                f"[seed-infer] warning: num_eval_envs={args.num_eval_envs}. "
                "For strict single-seed overfit tests, set num_eval_envs=1."
            )
    elif args.num_eval_envs > 1:
        raise ValueError(
            "Per-demo eval reset kwargs require num_eval_envs=1 to keep reset seeds aligned."
        )
