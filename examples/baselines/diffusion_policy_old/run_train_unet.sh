...
# Train script for examples/baselines/diffusion_policy/train_unet.py
...
UNET_DIMS="${UNET_DIMS:-128 128 256}"
N_GROUPS="${N_GROUPS:-8}"
...
echo "[train-unet] REPO_ROOT=$REPO_ROOT"
echo "[train-unet] DEMO_PATH=$DEMO_PATH"
echo "[train-unet] ENV_ID=$ENV_ID CONTROL_MODE=$CONTROL_MODE OBS_MODE=$OBS_MODE"
echo "[train-unet] TOTAL_ITERS=$TOTAL_ITERS BATCH_SIZE=$BATCH_SIZE MAX_EPISODE_STEPS=$MAX_EPISODE_STEPS"

python "$REPO_ROOT/examples/baselines/diffusion_policy/train_unet.py" "${ARGS[@]}"
