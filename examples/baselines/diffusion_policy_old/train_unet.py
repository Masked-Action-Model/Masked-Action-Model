...
def infer_control_mode_from_demo_json(demo_path):
    import json

    candidate_json_files = [demo_path[:-2] + "json"]
    base_path, _ = os.path.splitext(demo_path)
    for suffix in ["_normed", "_norm"]:
        if base_path.endswith(suffix):
            candidate_json_files.append(base_path[: -len(suffix)] + ".json")
    ...
    for json_file in candidate_json_files:
        ...
        env_info = demo_info.get("env_info", {})
        env_kwargs = env_info.get("env_kwargs", {})
        if "control_mode" in env_kwargs:
            return env_kwargs["control_mode"], json_file
        episodes = demo_info.get("episodes", [])
        if episodes and "control_mode" in episodes[0]:
            return episodes[0]["control_mode"], json_file

    checked_files = ", ".join(candidate_json_files)
    raise KeyError(f"Control mode metadata not found in any of: {checked_files}")
...
class SmallDemoDataset_DiffusionPolicy(Dataset):  # Load everything into memory
    def __init__(self, data_path, obs_process_fn, obs_space, include_rgb, include_depth, num_traj):
        ...
        print(
            f"Using generic action padding for control mode {args.control_mode}: "
            "linear extrapolation before trajectory, repeat-last after trajectory."
        )
    ...
    def _pad_actions_before(self, act_seq, pad_len):
        if pad_len <= 0:
            return act_seq
        return torch.cat([act_seq[0].repeat(pad_len, 1), act_seq], dim=0)
    ...
    def __getitem__(self, index):
        ...
        if start < 0:  # pad before the trajectory
            act_seq = self._pad_actions_before(act_seq, -start)
        if end > L:  # pad after the trajectory
            act_seq = torch.cat([act_seq, act_seq[-1].repeat(end - L, 1)], dim=0)
        assert (
...
class Agent(nn.Module):
    ...
    def set_action_denormalizer(self, mins: np.ndarray, maxs: np.ndarray, device):
        ...
        self.action_denorm_min = torch.as_tensor(
            mins, device=device, dtype=torch.float32
        )
        self.action_denorm_max = torch.as_tensor(
            maxs, device=device, dtype=torch.float32
        )
    ...
    def get_action(self, obs_seq):
        ...
        if self.action_denorm_dims > 0:
            ...
            action_seq[..., :d] = mins + 0.5 * (
                action_seq[..., :d].clamp(-1, 1) + 1.0
            ) * (maxs - mins)
        else:
            raise ValueError(
                "action_norm_path is required; no denormalization stats were loaded."
            )
        return action_seq
...
if __name__ == "__main__":
    ...
    if args.demo_path.endswith(".h5"):
        control_mode, metadata_json = infer_control_mode_from_demo_json(args.demo_path)
        assert (
            control_mode == args.control_mode
        ), (
            f"Control mode mismatched. Dataset has control mode {control_mode} "
            f"(from {metadata_json}), but args has control mode {args.control_mode}"
        )
