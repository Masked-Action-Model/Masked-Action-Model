...
from diffusion_policy.utils import build_state_obs_extractor, convert_obs, worker_init_fn
...
def main():
    ...
    if distributed:
        ddp_agent = DDP(agent, device_ids=[local_rank] if device.type == "cuda" else None, output_device=local_rank if device.type == "cuda" else None)
    else:
        ddp_agent = agent
