...
DATA_DIR="${DATA_DIR:-${BASE_DIR}/data/subgoal_mixed_random_points}"
LOG_DIR="${LOG_DIR:-${BASE_DIR}/logs}"
mkdir -p "${DATA_DIR}" "${LOG_DIR}"

RAW_DEMO_H5="${RAW_DEMO_H5:-demos/data_1/data_1.h5}"
RAW_DEMO_JSON="${RAW_DEMO_JSON:-demos/data_1/data_1.json}"
DEMO_PATH="${DEMO_PATH:-${DATA_DIR}/pickcube_subgoal_mixed_overfit5_train.h5}"
TRAIN_DEMO_METADATA_PATH="${TRAIN_DEMO_METADATA_PATH:-${DATA_DIR}/pickcube_subgoal_mixed_overfit5_train.json}"
EVAL_DEMO_PATH="${EVAL_DEMO_PATH:-${DATA_DIR}/pickcube_subgoal_mixed_overfit5_eval.h5}"
EVAL_DEMO_METADATA_PATH="${EVAL_DEMO_METADATA_PATH:-${DATA_DIR}/pickcube_subgoal_mixed_overfit5_eval.json}"
...
if [[ ! -f "${DEMO_PATH}" || ! -f "${EVAL_DEMO_PATH}" || ! -f "${TRAIN_DEMO_METADATA_PATH}" || ! -f "${EVAL_DEMO_METADATA_PATH}" ]]; then
...
export \
  EXP_NAME="${EXP_NAME:-PickCube_subgoal_mixed_overfit5_20k}" \
  ...
  PREPROCESSED_DATA_PREFIX="${PREPROCESSED_DATA_PREFIX:-pickcube_subgoal_mixed_overfit5}" \
  DEMO_PATH EVAL_DEMO_PATH TRAIN_DEMO_METADATA_PATH EVAL_DEMO_METADATA_PATH ACTION_NORM_PATH \
  ...
  DEMO_TYPE="${DEMO_TYPE:-pickcube_subgoal_mixed_overfit5}"
