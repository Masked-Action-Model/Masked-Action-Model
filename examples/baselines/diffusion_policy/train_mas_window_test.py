ALGO_NAME = "BC_Diffusion_rgbd_DiT_MASWindow_Test"
MAS_STEP_DIM = 8
...
from evaluate.evaluate_mas_window_test import evaluate_mas_window
...
if __name__ == "__main__":
    ...
    # ------------------------------------------------------------------------ #
    if args.test_demo_path is None or len(args.test_demo_path.strip()) == 0:
        args.test_demo_path = args.demo_path
    if args.num_demos != 10:
        raise ValueError(
            f"train_mas_window_test.py expects --num-demos=10, got {args.num_demos}."
        )
    if args.num_eval_demos != 5:
        raise ValueError(
            f"train_mas_window_test.py expects --num-eval-demos=5, got {args.num_eval_demos}."
        )
    eval_demo_path = args.test_demo_path
    overfit_eval_mode = (
        Path(eval_demo_path).resolve() == Path(args.demo_path).resolve()
    )
    eval_num_traj = args.num_demos if overfit_eval_mode else None
    ...
    eval_traj_ids = infer_eval_traj_ids_from_demo(
        eval_demo_path, num_traj=eval_num_traj
    )
    ...
    if len(eval_reset_seeds) > 0:
        ...
        assert args.num_eval_envs == 1, (
            "evaluation over specific demo seeds requires --num-eval-envs=1"
        )
    ...
    print(
        ...
        f"overfit_eval_mode={overfit_eval_mode}, "
    ...
    def evaluate_and_save_best(iteration):
        if iteration % args.eval_freq == 0:
            ...
            if len(eval_reset_seeds) > 0:
                ...
                per_traj_ce_results = []
                for local_idx, reset_seed in enumerate(eval_reset_seeds):
                    video_snapshot = None
                    if args.capture_video:
                        video_snapshot = snapshot_video_files(video_dir)
                    one_eval_data = subset_eval_data(eval_mam_data, [local_idx])
                    ...
                    if should_save_eval_artifacts:
                        one_metrics, progress_curve_records, rollout_records = one_eval_result
                    else:
                        one_metrics, rollout_records = one_eval_result
                        progress_curve_records = None
                    per_traj_ce_results.extend(
                        compute_control_error_results_from_rollouts(
                            eval_data=one_eval_data,
                    ...
                    success_at_end = bool(
                        np.asarray(one_metrics["success_at_end"]).reshape(-1)[0]
                    )
                    if args.capture_video:
                        if should_save_eval_artifacts:
                            if success_at_end:
                            ...
                            if (
                                ...
                                and len(progress_curve_records) > 0
                            ):
                                save_progress_curve_for_video(
                                    video_path=archived_video_path,
                                    video_dir=video_dir,
                                    timesteps=progress_curve_records[0]["steps"],
                                    progress=progress_curve_records[0]["progress"],
                                )
                        else:
                    ...
                    for k, v in one_metrics.items():
                        eval_metrics[k].append(np.asarray(v).reshape(-1)[0])
